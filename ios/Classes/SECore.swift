...
protocol SECoreProtocol {
    ...
    // encryption
    func encrypt(message: String, tag: String, password: String?) throws
        -> FlutterStandardTypedData?

    // encryption with base64 Public key
    func encryptWithPublicKey(message: String, publicKey: String) throws
        -> FlutterStandardTypedData?
...
class SECore: SECoreProtocol {
        ...
        -> FlutterStandardTypedData?
    {
        ...
        guard
            let newPublicKey = SecKeyCreateWithData(
                newPublicKeyData! as CFData, newPublicParams as CFDictionary, nil)
        else {
            throw CustomError.runtimeError("Cannot encrypt data")
        }
    ...
    func verify(tag: String, password: String?, plainText: String, signature: String) throws -> Bool
    {
        ...
        let verify = SecKeyVerifySignature(
            newPublicKey, SecKeyAlgorithm.ecdsaSignatureMessageX962SHA256, messageData as CFData,
            signatureData as CFData, nil)
        return verify
    }
